...
class HomeItemAdapter : PagingDataAdapter<HomeUiModel, ViewHolder>(HOME_UI_MODEL_COMPARATOR) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        homeUiModel.let {
            when (homeUiModel) {
                is HomeUiModel.PhotoItem -> (holder as PhotoViewHolder).bind(homeUiModel.photo)
                is HomeUiModel.AdBannerItem -> (holder as AdBannerViewHolder).bind(homeUiModel.text)
                else -> {
    ...
    companion object {
        private val HOME_UI_MODEL_COMPARATOR = object : DiffUtil.ItemCallback<HomeUiModel>() {
            override fun areItemsTheSame(oldItem: HomeUiModel, newItem: HomeUiModel): Boolean {
                return (oldItem is HomeUiModel.PhotoItem && newItem is HomeUiModel.PhotoItem &&
                        ...
                        (oldItem is HomeUiModel.AdBannerItem && newItem is HomeUiModel.AdBannerItem &&
